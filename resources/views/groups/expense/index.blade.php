...
.split-pill {
    ...
    padding:4px 10px;border-radius:20px;background:#f3f4f6;
...
.kieu-card {
    border:2px solid #e5e7eb;border-radius:10px;padding:10px;text-align:center;
...
.kieu-icon { font-size:20px;margin-bottom:4px; }
...
.mem-input {
    width:110px;padding:7px 10px;border:2px solid #e5e7eb;border-radius:8px;
...
/* Raw debt list */
.debt-item { padding:14px 22px;border-bottom:1px solid #f9fafb;display:flex;align-items:center;gap:14px; }
body.dark .debt-item { border-color:rgba(255,255,255,0.03); }
.debt-item:last-child { border-bottom:none; }
.di-info { flex:1; }
.di-desc { font-size:13px;font-weight:700;color:#1f2937; }
body.dark .di-desc { color:#e5e7eb; }
.di-meta { font-size:12px;color:#9ca3af; }
.di-amount { font-size:16px;font-weight:900;color:var(--danger); }
.di-status { padding:3px 9px;border-radius:12px;font-size:11px;font-weight:700; }
...
.btn-primary {
    ...
    color:white;font-size:13px;font-weight:700;border:none;cursor:pointer;text-decoration:none;transition:opacity .2s;
}
...
.btn-ghost {
    ...
    background:#f3f4f6;border:2px solid #e5e7eb;color:#6b7280;font-size:12px;font-weight:600;cursor:pointer;transition:background .2s;text-decoration:none;
}
                    ...
                    <div class="pi-top">
                        <div>
                            ...
                            <div class="pi-meta">
                                ...
                                · {{ match($p['kieu_chia']){'equal'=>'Chia đều','custom'=>'Tùy chỉnh',default=>'Theo %'} }}
                                @if($p['category']) · {{ $p['category'] }} @endif
                            </div>
                        </div>
...
function selectKieu(el, kieu) {
    ...
    const hint          = document.getElementById('kieu-hint');
    ...
    if (kieu === 'equal') {
        // Chia đều: chỉ hiện nhãn xác nhận, không cần nhập
        equalLabels.forEach(l => l.style.display='inline');
    } else if (kieu === 'custom') {
