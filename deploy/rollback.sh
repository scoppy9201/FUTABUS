#!/usr/bin/env bash
# FUTEBUS — roll the production stack back to the previous image release.
#
...
# health. Does NOT touch the database or any volume â€” data is preserved.
#
...
if HEALTH_URL="${HEALTH_URL:-http://127.0.0.1:${HTTP_PORT:-80}/up}" ./health-check.sh; then
    log "Rollback successful â€” previous release is healthy."
    exit 0
fi
