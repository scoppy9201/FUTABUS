#!/usr/bin/env bash
# FUTEBUS — zero-downtime-ish production deploy.
#
...
# restarts the stack and verifies health â€” rolling back automatically if the
# health check fails.
#
...
# 2. Registry login (token via stdin â€” never echoed) 
...
if "${COMPOSE[@]}" exec -T mysql sh -c \
    ...
    err "Database backup failed â€” aborting before any migration."
    rm -f "$BACKUP_FILE"
    exit 1
fi
...
if ! "${COMPOSE[@]}" run --rm --no-deps app php artisan migrate --force; then
    err "Migration failed â€” aborting deploy. Database backup preserved at $BACKUP_FILE."
    exit 1
fi
...
if HEALTH_URL="${HEALTH_URL:-http://127.0.0.1:${HTTP_PORT:-80}/up}" ./health-check.sh; then
    ok "Deploy healthy. Application is live on $APP_IMAGE."
    docker image prune -f >/dev/null 2>&1 || true   # dangling only â€” never -a
    exit 0
fi
